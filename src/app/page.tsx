...
const scriptFields = [
  ...
  ["الجمهور المستهدف", "محبو المنتجات الفاخرة"],
...
export default function Home() {
  ...
  function sendMessage(event: FormEvent<HTMLFormElement>) {
    ...
    setChat((current) => [...current, `أنت: ${message.trim()}`, "أسترا: هذه مساحة تجريبية لترتيب فكرتك."]);
    setMessage("");
  }
  ...
  return (
    <main id="home" className="site-shell">
      ...
      <header className="topbar">
        ...
        <nav className="desktop-nav" aria-label="التنقل الرئيسي"><a href="#home">الرئيسية</a><Link href="/create">إنشاء فيديو</Link><a href="#script">Script Studio</a><a href="#work">أعمالي</a></nav>
      ...
      <section className="tool-strip" aria-label="أقسام المنصة"><Link href="/create" className="tool-card active"><span className="tool-index">01</span><span><b>Video Models</b><small>محركات الفيديو</small></span><span className="arrow">↗</span></Link><Link href="/create" className="tool-card"><span className="tool-index">02</span><span><b>Text to Video</b><small>فكرة إلى فيديو</small></span><span className="arrow">↗</span></Link><Link href="/create" className="tool-card"><span className="tool-index">03</span><span><b>Image to Video</b><small>صورة إلى فيديو</small></span><span className="arrow">↗</span></Link><Link href="/script-studio" className="tool-card"><span className="tool-index">04</span><span><b>Script Studio</b><small>سكريبت دقيق</small></span><span className="arrow">↗</span></Link><Link href="/chat" className="tool-card"><span className="tool-index">05</span><span><b>Chat / Astra</b><small>مساعد إبداعي</small></span><span className="arrow">↗</span></Link></section>

      <section id="astra" className="content-section split-section"><div className="section-heading"><p className="eyebrow">أسترا · 02</p><h2>فكرة أوضح،<br /><em>بداية أقوى.</em></h2><p>مساحة حوار هادئة لترتيب الأفكار وتطويرها.</p></div><div className="chat-panel"><div className="panel-top"><span className="status-dot" /> أسترا متاحة للتجربة <span>واجهة فقط</span></div><div className="chat-messages">{chat.length ? chat.map((item, index) => <p key={`${item}-${index}`} className={item.startsWith("أنت") ? "user-message" : "astra-message"}>{item}</p>) : <div className="empty-chat"><span>✦</span><p>ما الذي تريد بناءه اليوم؟</p><small>اكتب فكرة أو اتجاهًا بصريًا.</small></div>}</div><form className="chat-input" onSubmit={sendMessage}><input value={message} onChange={(event) => setMessage(event.target.value)} placeholder="اكتب رسالتك هنا..." aria-label="رسالتك إلى أسترا" /><button type="submit" aria-label="إرسال الرسالة">↑</button></form></div></section>

      <section id="script" className="content-section script-section"><div className="section-heading"><p className="eyebrow">Script Studio · 03</p><h2>اكتب المشهد<br /><em>بتفاصيله.</em></h2><p>حوّل الفكرة الخام إلى مخطط تصوير واضح.</p></div><div className="script-panel"><form onSubmit={createScript} className="script-form">{scriptFields.map(([label, placeholder], index) => <label key={label}>{label}<input value={index === 0 ? scriptIdea : undefined} onChange={index === 0 ? (event) => setScriptIdea(event.target.value) : undefined} placeholder={placeholder} /></label>)}<div className="form-row"><label>اللغة<select defaultValue="العربية"><option>العربية</option><option>English</option></select></label><label>المدة<select defaultValue="20 ثانية"><option>15 ثانية</option><option>20 ثانية</option><option>30 ثانية</option></select></label></div><button className="button button-gold" type="submit">إنشاء السكريبت <span>✦</span></button></form>{script ? <pre className="script-result">{script}</pre> : <div className="script-placeholder"><span>✧</span><p>سيظهر السكريبت المنظم هنا</p><small>المشهد · الصورة · الكاميرا · الحوار</small></div>}</div></section>
